// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot.commands;

import edu.wpi.first.wpilibj.Timer;
import edu.wpi.first.wpilibj2.command.Command;
import frc.robot.Constants;
import frc.robot.Constants.PopcornConstants;
import frc.robot.subsystems.PopcornShooter;

/* You should consider using the more terse Command factories API instead https://docs.wpilib.org/en/stable/docs/software/commandbased/organizing-command-based.html#defining-commands */
public class PopcornShooterAuto extends Command {
  
  private final PopcornShooter shooter;
  private final Timer shooterTimer;
  private double shooterTime;

  public PopcornShooterAuto(PopcornShooter pShooter, double shooterTime) {
    // Use addRequirements() here to declare subsystem dependencies.
    shooter = pShooter;
    this.shooterTime = shooterTime;
    addRequirements(pShooter);
  
    shooterTimer = new Timer();
  }

  // Called when the command is initially scheduled.
  @Override
  public void initialize() {
    shooterTimer.start();
  }

  // Called every time the scheduler runs while the command is scheduled.
  @Override
  public void execute() {
    shooter.setElevator(Constants.PopcornConstants.ELEVATOR_SPEED);
  }

  // Called once the command ends or is interrupted.
  @Override
  public void end(boolean interrupted) {
    shooter.setElevator(0);
  }

  // Returns true when the command should end.
  @Override
  public boolean isFinished() {
    return shooterTimer.get() > shooterTime;
  }
}
